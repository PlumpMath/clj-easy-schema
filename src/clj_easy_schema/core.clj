...
(defn validate-enum-many
  "Make sure that each entry in <data> is one of the entries in <opts>"
  [opts data]
  (cond
    (nil? data) false
    (empty? opts) false
  ...
  (let [data (select-keys data (for [[field & tail] schema] field))]
    (loop [schm (first schema) left (rest schema)]
      (let [[field type & rst] schm
            datum (field data)
            args (unit-error (vec (concat [datum field type] rst)))
            mval (bind-error args validate-field)
            ]
        (cond
